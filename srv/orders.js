...
//    srv.on("UPDATE", "UpdateOrder", async (req) => {
      srv.on("UPDATE", "Orders", async (req) => {
        let returnData = await cds.transaction(req).run(
            ...
            if (resolve[0] == 0) {
                req.console.error(409, "Record not Found");
            }
        }).catch((err) => {
            console.log(err);
            req.error(err.code, err.message);
...
//    srv.on("DELETE", "DeleteOrder", async (req) => {
    ...
    srv.on("cancelOrder", async(req) => {
        ...
        if (resultsRead[0].Approved == false){
            ...
            returnOrder.message = `The Order placed by ${resultsRead[0].FirstName} ${resultsRead[0].LastName} was not cancel because was already approved`;
        }
